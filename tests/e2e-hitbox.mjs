...
const pages = [["/", null], ["/", tok], ["/private", tok], ["/sannin", tok], ["/sannin?view=maki", tok], ["/sannin/members", tok], ["/new", tok], ["/me", tok], ["/write", tok]];
const ok = [], bad = [];
for (const [path, t] of pages) {
  ...
  const label = path + (t ? "" : "（名乗る前）") + "：押せるものの当たりが、見えている字より広がっていない";
