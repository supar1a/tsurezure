...
/**
 * スペースの URL は道筋の一段目に直に置くので、ほかの画面と同じ名前になっては困る。
 * 実際に当たることはまずないが（14 字の生成なので）、当たったら黙って壊れる類なので塞ぐ。
 */
const TAKEN = new Set(["new", "me", "write", "rooms", "private", "post", "photo", "i", "s", "b", "api", "favicon.ico", "_next"]);

/**
 * 招待 URL の中身。
 *
 * スペースに入るための鍵はこれ一本きりなので、当てられない長さにしてある
